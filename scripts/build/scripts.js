...
const dexmaskrc = require('rc')('dexmask', {
...
async function bundleIt(buildConfiguration) {
  ...
  console.log(chalk.yellowBright(`bundle start: "${label}"`))
  await performBundle()
  console.log(chalk.greenBright(`bundle end: "${label}"`))
...
function getEnvironmentVariables({ devMode, testing }) {
  ...
  return {
    DEXMASK_DEBUG: devMode,
    DEXMASK_ENVIRONMENT: environment,
    METAMASK_VERSION: version,
    NODE_ENV: devMode ? 'development' : 'production',
    IN_TEST: testing ? 'true' : false,
    PUBNUB_SUB_KEY: process.env.PUBNUB_SUB_KEY || '',
    PUBNUB_PUB_KEY: process.env.PUBNUB_PUB_KEY || '',
    CONF: devMode ? dexmaskrc : {},
    SENTRY_DSN: process.env.SENTRY_DSN,
    SENTRY_DSN_DEV: dexmaskrc.SENTRY_DSN_DEV,
    INFURA_PROJECT_ID: testing
      ? '00000000000000000000000000000000'
      : dexmaskrc.INFURA_PROJECT_ID,
    SEGMENT_HOST: dexmaskrc.SEGMENT_HOST,
    // When we're in the 'production' environment we will use a specific key only set in CI
    // Otherwise we'll use the key from .dexmaskrc or from the environment variable. If
