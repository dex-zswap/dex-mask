...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync(address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        reject(new Error('DexMask Message: address field is required.'))
        return
      }
          ...
          case 'rejected':
            reject(
              ethErrors.provider.userRejectedRequest(
                'DexMask EncryptionPublicKey: User denied message EncryptionPublicKey.',
              ),
            )
            return
          default:
            reject(
              new Error(
                `DexMask EncryptionPublicKey: Unknown problem: ${JSON.stringify(
                  address,
                )}`,
              ),
            )
