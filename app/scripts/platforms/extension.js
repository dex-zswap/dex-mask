...
export default class ExtensionPlatform {
  ...
  _showConfirmedTransaction(txMeta, rpcPrefs) {
    ...
    const { chainId, hash } = txMeta;

    let url = getBlockExplorerLink(txMeta, rpcPrefs);

    if (!url && CHAINID_EXPLORE_MAP[chainId]) {
      url = createCustomExplorerLink(hash, CHAINID_EXPLORE_MAP[chainId]);
    }
