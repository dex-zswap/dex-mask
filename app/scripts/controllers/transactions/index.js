...
import { TRANSACTION_NO_CONTRACT_ERROR_KEY } from '@view/helpers/constants/error-keys';
import { hexWEIToDecGWEI } from '@view/helpers/utils/conversions.util';
...
export default class TransactionController extends EventEmitter {
  ...
  async getCommonConfiguration(fromAddress) {
    ...
    if (type !== NETWORK_TYPE_RPC && !SPECIAL_PROVIDER_TYPES.includes(type)) {
      return new Common({
        chain: type,
        hardfork,
      });
              ...
              return reject(
                cleanErrorStack(
                  ethErrors.provider.userRejectedRequest(
                    'DexMask Tx Signature: User denied transaction signature.',
                  ),
                ),
              );
              ...
              return reject(
                cleanErrorStack(
                  ethErrors.rpc.internal(
                    `DexMask Tx Signature: Unknown problem: ${JSON.stringify(
  ...
  */
  async signTransaction(txId) {
    ...
    // add r,s,v values for provider request purposes see createDexmaskMiddleware
