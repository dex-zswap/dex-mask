...
import { PRIORITY_STATUS_HASH, PENDING_STATUS_HASH } from '@view/helpers/constants/transactions';
import { hexToDecimal } from '@view/helpers/utils/conversions.util';
import txHelper from '@view/helpers/utils/tx-helper';
import { TRANSACTION_STATUSES, TRANSACTION_TYPES } from '@shared/constants/transaction';
...
export const incomingTxListSelector = state => {
  ...
  return Object.values(state.metamask.incomingTransactions).filter(tx => tx.txParams.to === selectedAddress && transactionMatchesNetwork(tx, chainId, network));
};
...
export const selectedAddressTxListSelector = createSelector(getSelectedAddress, currentNetworkTxListSelector, (selectedAddress, transactions = []) => {
  return transactions.filter(({
    txParams
  }) => txParams.from === selectedAddress);
});
export const unapprovedMessagesSelector = createSelector(unapprovedMsgsSelector, unapprovedPersonalMsgsSelector, unapprovedDecryptMsgsSelector, unapprovedEncryptionPublicKeyMsgsSelector, unapprovedTypedMessagesSelector, deprecatedGetCurrentNetworkId, getCurrentChainId, (unapprovedMsgs = {}, unapprovedPersonalMsgs = {}, unapprovedDecryptMsgs = {}, unapprovedEncryptionPublicKeyMsgs = {}, unapprovedTypedMessages = {}, network, chainId) => txHelper({}, unapprovedMsgs, unapprovedPersonalMsgs, unapprovedDecryptMsgs, unapprovedEncryptionPublicKeyMsgs, unapprovedTypedMessages, network, chainId) || []);
export const transactionSubSelector = createSelector(unapprovedMessagesSelector, incomingTxListSelector, (unapprovedMessages = [], incomingTxList = []) => {
  return unapprovedMessages.concat(incomingTxList);
});
export const transactionsSelector = createSelector(transactionSubSelector, selectedAddressTxListSelector, (subSelectorTxList = [], selectedAddressTxList = []) => {
...
export const nonceSortedTransactionsSelector = createSelector(transactionsSelector, (transactions = []) => {
  ...
  transactions.forEach(transaction => {
    ...
    if (typeof nonce === 'undefined' || type === TRANSACTION_TYPES.INCOMING) {
      ...
      const previousPrimaryIsNetworkFailure = nonceProps.primaryTransaction.status === TRANSACTION_STATUSES.FAILED && nonceProps.primaryTransaction?.txReceipt?.status !== '0x0';
      const currentTransactionIsOnChainFailure = transaction?.txReceipt?.status === '0x0';

      if (status === TRANSACTION_STATUSES.CONFIRMED || currentTransactionIsOnChainFailure || previousPrimaryIsNetworkFailure || txTime > primaryTxTime && status in PRIORITY_STATUS_HASH) {
...
export const nonceSortedPendingTransactionsSelector = createSelector(nonceSortedTransactionsSelector, (transactions = []) => transactions.filter(({
  primaryTransaction
...
export const nonceSortedCompletedTransactionsSelector = createSelector(nonceSortedTransactionsSelector, (transactions = []) => transactions.filter(({
  primaryTransaction
}) => !(primaryTransaction.status in PENDING_STATUS_HASH)).reverse());
export const submittedPendingTransactionsSelector = createSelector(transactionsSelector, (transactions = []) => transactions.filter(transaction => transaction.status === TRANSACTION_STATUSES.SUBMITTED));