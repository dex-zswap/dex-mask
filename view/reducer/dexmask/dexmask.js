...
import { accountsWithSendEtherInfoSelector, checkNetworkAndAccountSupports1559, getAddressBook } from '@view/selectors';
...
export default function reduceDexmask(state = {}, action) {
  ...
  switch (action.type) {
    ...
    case actionConstants.SET_ACCOUNT_LABEL:
      {
        ...
        const id = {};
        id[account] = { ...dexmaskState.identities[account],
          name
        };
        const identities = { ...dexmaskState.identities,
          ...id
        };
        return Object.assign(dexmaskState, {
          identities
        });
      }
    ...
    case actionConstants.UPDATE_PREFERENCES:
      {
        return { ...dexmaskState,
          preferences: { ...dexmaskState.preferences,
            ...action.payload
          }
        };
      }
...
export function updateTransactionGasFees({
  ...
  return async dispatch => {
    ...
    if (gasPrice) {
      dispatch(setCustomGasPrice(toHexWei(txParamsCopy.gasPrice, expectHexWei)));
      txParamsCopy.gasPrice = toHexWei(gasPrice, expectHexWei);
    } else if (maxFeePerGas && maxPriorityFeePerGas) {
      txParamsCopy.maxFeePerGas = toHexWei(maxFeePerGas, expectHexWei);
      txParamsCopy.maxPriorityFeePerGas = addHexPrefix(decGWEIToHexWEI(maxPriorityFeePerGas));
    }

    const updatedTx = { ...transaction,
      txParams: txParamsCopy
    };
    const customGasLimit = isHexString(addHexPrefix(gasLimit)) ? addHexPrefix(gasLimit) : addHexPrefix(gasLimit.toString(16));
...
export const getWeb3ShimUsageAlertEnabledness = state => getAlertEnabledness(state)[ALERT_TYPES.web3ShimUsage];
...
export function getIsGasEstimatesLoading(state) {
  ...
  const isEIP1559TolerableEstimateType = gasEstimateType === GAS_ESTIMATE_TYPES.FEE_MARKET || gasEstimateType === GAS_ESTIMATE_TYPES.ETH_GASPRICE;
  const isGasEstimatesLoading = gasEstimateType === GAS_ESTIMATE_TYPES.NONE || networkAndAccountSupports1559 && !isEIP1559TolerableEstimateType || !networkAndAccountSupports1559 && gasEstimateType === GAS_ESTIMATE_TYPES.FEE_MARKET;
