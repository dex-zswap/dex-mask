...
import { fetchSwapsFeatureFlags, fetchSwapsGasPrices, getSwapsLivenessForNetwork, isContractAddressValid } from '@pages/swaps/swaps.util';
import { getGasFeeEstimates } from '@reducer/dexmask/dexmask';
import { CONTRACT_DATA_DISABLED_ERROR, ERROR_FETCHING_QUOTES, QUOTES_NOT_AVAILABLE_ERROR, SWAPS_FETCH_ORDER_CONFLICT, SWAP_FAILED_ERROR } from '@shared/constants/swaps';
import { TRANSACTION_TYPES } from '@shared/constants/transaction';
import { conversionLessThan } from '@shared/modules/conversion.utils';
import { AWAITING_SWAP_ROUTE, BUILD_QUOTE_ROUTE, LOADING_QUOTES_ROUTE, SWAPS_ERROR_ROUTE, SWAPS_MAINTENANCE_ROUTE } from '@view/helpers/constants/routes';
import { addHexes, decGWEIToHexWEI, decimalToHex, getValueFromWeiHex, hexWEIToDecGWEI } from '@view/helpers/utils/conversions.util';
import { calcTokenAmount } from '@view/helpers/utils/token-util';
import { checkNetworkAndAccountSupports1559, getCurrentChainId, getHardwareWalletType, getSelectedAccount, getSwapsDefaultToken, getTokenExchangeRates, getUSDConversionRate, isHardwareWallet } from '@view/selectors';
import { addToken, addUnapprovedTransaction, cancelTx, fetchAndSetQuotes, forceupdateDexmaskState, resetBackgroundSwapsState, resetSwapsPostFetchState, setApproveTxId, setBackgroundSwapRouteState, setInitialGasEstimate, setSelectedQuoteAggId, setSwapsErrorKey, setSwapsLiveness, setSwapsTxGasLimit, setSwapsTxGasPrice, setTradeTxId, stopPollingForQuotes, updateAndApproveTx, updateTransaction } from '@view/store/actions';
...
export const getSwapsFeatureIsLive = state => state.metamask.swapsState.swapsFeatureIsLive;
export const getUseNewSwapsApi = state => state.metamask.swapsState.useNewSwapsApi;
export const getSwapsQuoteRefreshTime = state => state.metamask.swapsState.swapsQuoteRefreshTime;
...
export const getCustomMaxFeePerGas = state => state.metamask.swapsState.customMaxFeePerGas;
export const getCustomMaxPriorityFeePerGas = state => state.metamask.swapsState.customMaxPriorityFeePerGas;
export const getSwapsUserFeeLevel = state => state.metamask.swapsState.swapsUserFeeLevel;
...
export const getUsedQuote = state => getSelectedQuote(state) || getTopQuote(state); // Compound selectors
...
export { clearSwapsState, setAggregatorMetadata, setBalanceError, setFetchingQuotes, setFromToken as setSwapsFromToken, setQuotesFetchStartTime as setSwapQuotesFetchStartTime, setTopAssets, setToToken as setSwapToToken, swapCustomGasModalPriceEdited, swapCustomGasModalLimitEdited, swapCustomGasModalClosed };
...
export const fetchSwapsLiveness = () => {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      swapsLivenessForNetwork = getSwapsLivenessForNetwork(swapsFeatureFlags, getCurrentChainId(getState()));
    } catch (error) {
...
export const fetchQuotesAndSetQuoteState = (history, inputValue, maxSlippage, metaMetricsEvent) => {
  return async (dispatch, getState) => {
    ...
    try {
      const swapsFeatureFlags = await fetchSwapsFeatureFlags();
      swapsLivenessForNetwork = getSwapsLivenessForNetwork(swapsFeatureFlags, chainId);
    } catch (error) {
      log.error('Failed to fetch Swaps liveness, defaulting to false.', error);
    }
    ...
    const fetchParamsFromToken = fetchParams?.metaData?.sourceTokenInfo?.symbol === swapsDefaultToken.symbol ? swapsDefaultToken : fetchParams?.metaData?.sourceTokenInfo;
    ...
    if (fromTokenAddress && fromTokenSymbol !== swapsDefaultToken.symbol && !contractExchangeRates[fromTokenAddress] && fromTokenBalance && new BigNumber(fromTokenBalance, 16).gt(0)) {
    ...
    const sourceTokenInfo = swapsTokens?.find(({
      address
    }) => address === fromTokenAddress) || selectedFromToken;
    const destinationTokenInfo = swapsTokens?.find(({
      address
    ...
    const networkAndAccountSupports1559 = checkNetworkAndAccountSupports1559(state);
    metaMetricsEvent({
...
export const signAndSendTransactions = (history, metaMetricsEvent) => {
  return async (dispatch, getState) => {
    ...
    if (networkAndAccountSupports1559) {
      ...
      baseAndPriorityFeePerGas = addHexes(decEstimatedBaseFee, maxPriorityFeePerGas);
    }

    const usedQuote = getUsedQuote(state);
    const usedTradeTxParams = usedQuote.trade;
    const estimatedGasLimit = new BigNumber(usedQuote?.gasEstimate || `0x0`, 16);
    const estimatedGasLimitWithMultiplier = estimatedGasLimit.times(usedQuote?.gasMultiplier || FALLBACK_GAS_MULTIPLIER, 10).round(0).toString(16);
    const maxGasLimit = customSwapsGas || (usedQuote?.gasEstimate ? estimatedGasLimitWithMultiplier : `0x${decimalToHex(usedQuote?.maxGas || 0)}`);
    ...
    const gasEstimateTotalInUSD = getValueFromWeiHex({
      value: calcGasTotal(totalGasLimitEstimate, networkAndAccountSupports1559 ? baseAndPriorityFeePerGas : usedGasPrice),
      toCurrency: 'usd',
    ...
    if (approveTxParams) {
      ...
      const approveTxMeta = await dispatch(addUnapprovedTransaction({ ...approveTxParams,
        amount: '0x0'
      }, 'dexmask'));
      await dispatch(setApproveTxId(approveTxMeta.id));
      finalApproveTxMeta = await dispatch(updateTransaction({ ...approveTxMeta,
    ...
    const tradeTxMeta = await dispatch(addUnapprovedTransaction(usedTradeTxParams, 'dexmask'));
    ...
    const finalTradeTxMeta = await dispatch(updateTransaction({ ...tradeTxMeta,
      estimatedBaseFee: decEstimatedBaseFee,
      sourceTokenSymbol: sourceTokenInfo.symbol,
    ...
    try {
      ...
      const errorKey = e.message.includes('EthAppPleaseEnableContractData') ? CONTRACT_DATA_DISABLED_ERROR : SWAP_FAILED_ERROR;
      await dispatch(setSwapsErrorKey(errorKey));
      history.push(SWAPS_ERROR_ROUTE);
      return;
    }

    await forceupdateDexmaskState(dispatch);
  };
};
