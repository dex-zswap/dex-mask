...
const Menu = ({
  ...
  const {
    attributes,
    styles
  } = usePopper(anchorElement, popperElement, popperOptions);
  return createPortal(<>
      <div className="menu__background" onClick={onHide} />
      <div className={classnames('menu__container', className)} ref={setPopperElement} style={styles.popper} {...attributes.popper}>
        {children}
      </div>
