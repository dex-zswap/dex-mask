...
export default class ButtonGroup extends PureComponent {
  ...
  renderButtons() {
    ...
    return React.Children.map(children, (child, index) => {
      return child && <button role={variant === 'radiogroup' ? 'radio' : undefined} aria-checked={index === this.state.activeButtonIndex} className={classnames('button-group__button', child.props.className, {
      ...
      })} data-testid={`button-group__button${index}`} onClick={() => {
        this.handleButtonClick(index);
        child.props.onClick?.();
      }} disabled={disabled || child.props.disabled} key={index}>
  ...
  render() {
    ...
    })} role={variant === 'radiogroup' ? 'radiogroup' : undefined} style={style}>
        {this.renderButtons()}
      </div>;
  }
