...
export default function ListItem({
  ...
  const primaryClassName = classnames('list-item', className, subtitle || children ? '' : 'list-item--single-content-row');
  return <div className={primaryClassName} onClick={onClick} data-testid={dataTestId} role="button" tabIndex={0} onKeyPress={event => {
    if (event.key === 'Enter') {
      ...
      <div className="list-item__heading">
        ...
        {titleIcon && <div className="list-item__heading-wrap">{titleIcon}</div>}
      </div>
