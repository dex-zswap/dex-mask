...
export default function Selector({
  ...
  const selectedLabel = useMemo(() => options.find(({
    value
  }) => value === selectedValue)?.label || '', [options, selectedValue]);
  return <div className={classnames(['selector-component', small && 'small', show ? 'menu-opened' : 'menu-hidden', className])}>
      ...
      {show && <>
          ...
          label,
          value
        }, index) => <div className={classnames(['select-option', selectedValue === value ? 'select-option-active-color' : ''])} onClick={e => {
          e.stopPropagation();
          onChange(value);
        }} key={value}>
