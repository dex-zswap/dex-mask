...
import { getEthConversionFromWeiHex, getValueFromWeiHex } from '@view/helpers/utils/conversions.util';
...
export default class TransactionActivityLog extends PureComponent {
  ...
  handleActivityClick = activity => {
    ...
    if (!etherscanUrl && CHAINID_EXPLORE_MAP[activity.chainId]) {
      etherscanUrl = createCustomExplorerLink(activity.hash, CHAINID_EXPLORE_MAP[activity.chainId]);
    }
  ...
  renderInlineRetry(index) {
    ...
    return isEarliestNonce && status !== CONFIRMED_STATUS && index === inlineRetryIndex ? <div className="transaction-activity-log__action-link" onClick={onRetry}>
  ...
  renderInlineCancel(index) {
    ...
    return isEarliestNonce && status !== CONFIRMED_STATUS && index === inlineCancelIndex ? <div className="transaction-activity-log__action-link" onClick={onCancel}>
  ...
  renderActivity(activity, index) {
    ...
    const ethValue = index === 0 ? `${getValueFromWeiHex({
      value,
      fromCurrency: 'ETH',
      toCurrency: 'ETH',
      conversionRate,
      numberOfDecimals: 6
    })} ${nativeCurrency}` : getEthConversionFromWeiHex({
      value,
      fromCurrency: 'ETH',
      conversionRate,
      numberOfDecimals: 3
    ...
    return <div key={index} className="transaction-activity-log__activity">
        <TransactionActivityLogIcon className="transaction-activity-log__activity-icon" eventKey={eventKey} />
        <div className="transaction-activity-log__entry-container">
          <div className="transaction-activity-log__activity-text" title={activityText} onClick={() => this.handleActivityClick(activity)}>
            {activityText}
          </div>
  ...
  render() {
    ...
    return <div className={classnames('transaction-activity-log', className)}>
