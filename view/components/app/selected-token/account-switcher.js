...
export default function AccountSwitcher() {
  ...
  const selectedAddress = useMemo(() => selectedIdentity.address, [selectedIdentity.address]);
        ...
        >
          {
            accounts.map((account) => (
              <MenuItem wrapChildAsDiv key={account.address} onClick={() => selectAccount(account.address)}>
                <div className="account-list-option flex space-between items-center">
                  <div className="left-avatar flex items-center">
                    ...
                    <p className={classnames('account-name', selectedAddress === account.address && 'active')}>{account.name}</p>
                  </div>
                  <p className={classnames('account-address', selectedAddress === account.address && 'active')}>{shortenAddress(account.address, 9, -6)}</p>
                </div>
              </MenuItem>
            ))
          }
