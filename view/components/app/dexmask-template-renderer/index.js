...
const DexMaskTemplateRenderer = ({
  ...
  if (!sections) {
    ...
    return <Element {...sections.props}>
        {typeof sections.children === 'object' ? <DexMaskTemplateRenderer sections={sections.children} /> : sections?.children}
      </Element>;
  ...
  return <>
      ...
      if (typeof child === 'string') {
        ...
        if (typeof child?.children === 'object') {
          ...
          allChildren.push(<DexMaskTemplateRenderer sections={child} key={child.key} />);
        } else {
          // Otherwise render the element.
          const Element = getElement(child);
          allChildren.push(<Element key={child.key} {...child.props}>
                {child?.children}
              </Element>);
        }
      }
...
const ValidChildren = PropTypes.oneOfType([PropTypes.string, PropTypes.shape(SectionShape), PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.shape(SectionShape), PropTypes.string]))]);
