...
import { NETWORKS_FORM_ROUTE, NETWORKS_ROUTE } from '@view/helpers/constants/routes';
...
class NetworkDropdown extends Component {
  ...
  renderCustomRpcList(rpcListDetail, provider) {
    ...
    return reversedRpcListDetail.map(entry => {
      ...
      return <DropdownMenuItem key={`common${rpcUrl}`} closeMenu={() => this.props.hideNetworkDropdown()} onClick={() => {
        ...
        padding: '12px 0'
          ...
          {isCurrentRpcTarget ? <i className="fa fa-check" /> : <div className="network-check__transparent">✓</div>}
          <ColorIndicator color={COLORS.UI2} size={SIZES.LG} type={ColorIndicator.TYPES.FILLED} borderColor={isCurrentRpcTarget ? COLORS.WHITE : COLORS.UI2} />
          <span className="network-name-item" style={{
          color: isCurrentRpcTarget ? '#ffffff' : '#9b9b9b'
        }}>
            {nickname || rpcUrl}
          </span>
          {isCurrentRpcTarget ? null : <i className="fa fa-times delete" onClick={e => {
          e.stopPropagation();
  ...
  renderNetworkEntry(network) {
    ...
    return <DropdownMenuItem key={network} closeMenu={this.props.hideNetworkDropdown} onClick={() => this.handleClick(network)} style={DROP_DOWN_MENU_ITEM_STYLE}>
        {providerType === network ? <i className="fa fa-check" /> : <div className="network-check__transparent">✓</div>}
        <ColorIndicator color={network} size={SIZES.LG} type={ColorIndicator.TYPES.FILLED} borderColor={providerType === network ? COLORS.WHITE : network} />
        <span className="network-name-item" style={{
  ...
  renderDrop() {
    ...
    return <Dropdown isOpen={isOpen} onClickOutside={event => {
      const {
        classList
      } = event.target;
      ...
      if (notToggleElementIndex === -1) {
    ...
    }} containerClassName={['network-droppo', isOpen && 'network-droppo__wrapper'].join(' ')} style={{}}>
        ...
        <DropdownMenuItem closeMenu={() => this.props.hideNetworkDropdown()} onClick={() => {
        this.props.history.push(NETWORKS_FORM_ROUTE);
        setSelectedSettingsRpcUrl('');
        setNetworksTabAddMode(true);
      }} style={DROP_DOWN_MENU_ITEM_STYLE}>
          {activeNetwork === 'custom' ? <i className="fa fa-check" /> : <div className="network-check__transparent">✓</div>}
          <ColorIndicator type={ColorIndicator.TYPES.FILLED} color={COLORS.TRANSPARENT} borderColor={COLORS.UI2} size={SIZES.LG} />
          <span className="network-name-item" style={{
  ...
  renderMenu() {
    ...
    const isOpen = this.props.networkDropdownOpen;
    return <>
        {isOpen ? <Menu className="network-droppo__menu" anchorElement={triggerEl} onHide={hideNetworkDropdown}>
            ...
            <MenuItem onClick={() => {
          this.props.history.push(getEnvironmentType() === ENVIRONMENT_TYPE_FULLSCREEN ? NETWORKS_ROUTE : NETWORKS_FORM_ROUTE);
          setSelectedSettingsRpcUrl('');
          setNetworksTabAddMode(true);
        }}>
