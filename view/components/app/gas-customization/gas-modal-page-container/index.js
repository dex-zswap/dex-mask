...
import { resetCustomData, setCustomGasLimit, setCustomGasPrice } from '@reducer/gas/gas.duck';
import { ASSET_TYPES, getGasLimit, getGasPrice, getSendAmount, getSendAsset, getSendMaxModeState, updateGasLimit, updateGasPrice, useCustomGas } from '@reducer/send';
import { GAS_LIMITS } from '@shared/constants/gas';
import { TRANSACTION_STATUSES } from '@shared/constants/transaction';
import { addHexes, getValueFromWeiHex, hexWEIToDecGWEI, subtractHexWEIsToDec, sumHexWEIsToRenderableFiat } from '@view/helpers/utils/conversions.util';
import { formatETHFee } from '@view/helpers/utils/formatters';
import { conversionRateSelector as getConversionRate, getAveragePriceEstimateInHexWEI, getBasicGasEstimateLoadingStatus, getCurrentCurrency, getCurrentEthBalance, getCustomGasLimit, getCustomGasPrice, getDefaultActiveButtonIndex, getIsCustomNetworkGasPriceFetched, getIsGasEstimatesFetched, getIsMainnet, getIsTestnet, getRenderableBasicEstimateData, getShouldShowFiat, isCustomPriceExcessive, isCustomPriceSafe, isCustomPriceSafeForCustomNetwork } from '@view/selectors';
import { createRetryTransaction, createSpeedUpTransaction, hideModal, hideSidebar } from '@view/store/actions';
...
const mapStateToProps = (state, ownProps) => {
  ...
  const txParams = selectedTransaction?.txParams ? selectedTransaction.txParams : {
  ...
  const customModalGasLimitInHex = getCustomGasLimit(state) || currentGasLimit || GAS_LIMITS.SIMPLE;
  const customGasTotal = calcGasTotal(customModalGasLimitInHex, customModalGasPriceInHex);
  const gasButtonInfo = getRenderableBasicEstimateData(state, customModalGasLimitInHex);
  const currentCurrency = getCurrentCurrency(state);
  const conversionRate = getConversionRate(state);
  const newTotalFiat = sumHexWEIsToRenderableFiat([value, customGasTotal], currentCurrency, conversionRate);
  const {
    hideBasic
  ...
  const newTotalEth = maxModeOn && asset.type === ASSET_TYPES.NATIVE ? sumHexWEIsToRenderableEth([balance, '0x0']) : sumHexWEIsToRenderableEth([value, customGasTotal]);
  const sendAmount = maxModeOn && asset.type === ASSET_TYPES.NATIVE ? subtractHexWEIsFromRenderableEth(balance, customGasTotal) : sumHexWEIsToRenderableEth([value, '0x0']);
  ...
  const customNetworkEstimateWasFetched = getIsCustomNetworkGasPriceFetched(state);
  let customPriceIsSafe = true;

  if ((isMainnet || process.env.IN_TEST) && isGasEstimate) {
    customPriceIsSafe = isCustomPriceSafe(state);
  } else if (!(isMainnet || process.env.IN_TEST || isTestnet) && customNetworkEstimateWasFetched) {
  ...
  return {
    ...
    infoRowProps: {
      originalTotalFiat: sumHexWEIsToRenderableFiat([value, customGasTotal], currentCurrency, conversionRate),
...
const mapDispatchToProps = dispatch => {
  ...
  return {
    ...
    updateTransactionGasFees: gasFees => {
      dispatch(updateTransactionGasFees({ ...gasFees,
        expectHexWei: true
      }));
    },
...
const mergeProps = (stateProps, dispatchProps, ownProps) => {
  ...
  return { ...stateProps,
    ...
    disableSave: insufficientBalance || isSpeedUp && customGasPrice === 0 || customGasLimit < Number(MIN_GAS_LIMIT_DEC)
  };
};
