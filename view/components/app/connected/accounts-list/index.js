...
export default class ConnectedAccountsList extends PureComponent {
  ...
  renderUnconnectedAccount() {
    ...
    return <ConnectedAccountsListItem className="connected-accounts-list__row--highlight" address={address} name={`${name} (…${address.substr(-4, 4)})`} status={t('statusNotConnected')} action={<a className="connected-accounts-list__account-status-link" onClick={() => connectAccount(accountToConnect.address)}>
            {t('connect')}
          </a>} />;
  }
  ...
  renderListItemOptions(address) {
    ...
    return <ConnectedAccountsListOptions onHideOptions={this.hideAccountOptions} onShowOptions={this.showAccountOptions.bind(null, address)} show={accountWithOptionsShown === address}>
        <MenuItem iconClassName="disconnect-icon" onClick={this.disconnectAccount}>
  ...
  renderListItemAction(address) {
    ...
    return <a className="connected-accounts-list__account-status-link" onClick={() => this.switchAccount(address)}>
  ...
  render() {
    ...
    return <>
        <main className="connected-accounts-list">
          ...
          return <ConnectedAccountsListItem key={address} address={address} name={`${name} (…${address.substr(-4, 4)})`} status={index === 0 ? t('active') : null} options={shouldRenderListOptions ? this.renderListItemOptions(address) : null} action={address === selectedAddress ? null : this.renderListItemAction(address)} />;
        })}
        </main>
      </>;
  }
