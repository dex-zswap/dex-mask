...
export default class ConnectedSitesList extends Component {
  ...
  render() {
    ...
    return <main className="connected-sites-list__content-rows">
        {connectedDomains.map(domain => <div key={domain.origin} className="connected-sites-list__content-row">
            ...
            <i className="fas fa-trash-alt connected-sites-list__trash" title={t('disconnect')} onClick={() => onDisconnect(domain.origin)} />
          </div>)}
      </main>;
  }
  ...
  getDomainDisplayName(domain) {
    ...
    return this.props.domainHostCount[domain.host] > 1 ? domain.origin : stripHttpSchemes(domain.origin);
  }
