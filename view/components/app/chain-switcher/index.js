...
import { BSC_MAINNET, DEX_MAINNET, MAINNET, NETWORK_TYPE_RPC, DEFAULT_NETWORK_LIST } from '@shared/constants/network';
...
const SelectorOption = props => {
  return <div className={classnames('chain-option-item flex items-center', props.value.toLowerCase())}>
...
class ChainSwitcher extends Component {
  ...
  defaultChains = DEFAULT_NETWORK_LIST.map(({
    chainId,
    provider,
    label
  }) => {
  ...
  renderCurrentChain() {
    return <div className="chain-switcher__current-chain">
        {this.getNetworkName()}
      </div>;
  }
  ...
  renderDefaultChains() {
    ...
    const providerName = provider.type;
    return <div className="chain-switcher__default-chains base-width">
        {
        /* <div
  ...
  renderNetWorkMenu() {
    ...
    return <Menu className="network-droppo__menu" anchorElement={this.triggerEl} onHide={() => this.toggleNetworkDrop()}>
...
export default compose(withRouter, connect(mapStateToProps, mapDispatchToProps))(ChainSwitcher);