...
import { getCurrentChainId, getCurrentKeyring, getRpcPrefsForCurrentProvider, getSelectedIdentity } from '@view/selectors';
...
export default function AccountOptionsMenu({
  ...
  return <Menu anchorElement={anchorElement} className="account-options-menu" onHide={onClose}>
      {!process.env.DEXMASK_DEBUG ? null : <MenuItem onClick={() => {
      global.platform.openExtensionInBrowser();
      onClose();
    }} iconClassName="account-options-menu__expand-view">
          {t('expandView')}
        </MenuItem>}
      <MenuItem data-testid="account-options-menu__account-details" onClick={() => {
      dispatch(showModal({
      ...
      onClose();
    }} subtitle={blockExplorerUrlSubTitle ? <span className="account-options-menu__explorer-origin">
              {blockExplorerUrlSubTitle}
            </span> : null} iconClassName="account-options-menu__explorer-origin">
        {rpcPrefs.blockExplorerUrl ? t('viewinExplorer', [providerType]) : isMainnet ? t('viewOnEtherscan') : t('viewinExplorer', [providerType])}
      </MenuItem>
      <MenuItem data-testid="account-options-menu__connected-sites" onClick={() => {
      history.push(CONNECTED_ROUTE);
      onClose();
    }} iconClassName="account-options-menu__connected-sites">
        {t('connectedSites')}
      </MenuItem>
      {isRemovable ? <MenuItem data-testid="account-options-menu__remove-account" onClick={() => {
      dispatch(showModal({
