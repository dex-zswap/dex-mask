...
import { getAssetImages, getShouldHideZeroBalanceTokens, getTokenDisplayOrders } from '@view/selectors';
export default function TokenList({
  ...
  const shouldHideZeroBalanceTokens = useSelector(getShouldHideZeroBalanceTokens); // use `isEqual` comparison function because the token array is serialized
  ...
  const tokenSorted = useMemo(() => {
    ...
    return tokenDisplayOrders.map(address => tokensWithBalances.find(({
      address: tokenAddress
    }) => address === tokenAddress)).concat(tokensWithBalances.filter(({
      address
    }) => !tokenDisplayOrders.includes(address)));
