...
import { ADD_TOKEN_ROUTE, ASSET_ROUTE, BUILD_QUOTE_ROUTE, CONFIRMATION_V_NEXT_ROUTE, CONFIRM_ADD_SUGGESTED_TOKEN_ROUTE, CONFIRM_ADD_TOKEN_ROUTE, CONFIRM_TRANSACTION_ROUTE, CONNECT_ROUTE, CROSSCHAIN_ROUTE, DEFAULT_ROUTE, INITIALIZE_ROUTE, INITIALIZE_UNLOCK_ROUTE, LOCK_ROUTE, NEW_ACCOUNT_ROUTE, RECIVE_TOKEN_ROUTE, RESTORE_VAULT_ROUTE, REVEAL_SEED_ROUTE, SEND_ROUTE, SETTINGS_ROUTE, SWAPS_ROUTE, TRADE_ROUTE, UNLOCK_ROUTE } from '@view/helpers/constants/routes';
...
export default class Routes extends Component {
  ...
  renderRoutes() {
    ...
    const routes = <Switch>
        ...
        <Authenticated path={CONFIRMATION_V_NEXT_ROUTE} component={ConfirmationPage} />
    ...
    if (autoLockTimeLimit > 0) {
      return <IdleTimer onAction={setLastActiveTime} throttle={1000}>
          {routes}
        </IdleTimer>;
    }
  ...
  hideAppHeader() {
    ...
    const isUnlockingPage = Boolean(matchPath(location.pathname, {
      path: INITIALIZE_UNLOCK_ROUTE,
      exact: false
    })) || Boolean(matchPath(location.pathname, {
      path: UNLOCK_ROUTE,
      exact: false
    }));

    if (isInitializing && !this.onInitializationUnlockPage() || isUnlockingPage) {
      return true;
    }
    ...
    const isHandlingAddEthereumChainRequest = Boolean(matchPath(location.pathname, {
  ...
  render() {
    ...
    const sidebarShouldClose = sidebarTransaction && !sidebarTransaction.status === TRANSACTION_STATUSES.FAILED && !submittedPendingTransactions.find(({
      id
    }) => id === sidebarTransaction.id);
    const {
      os,
      browser
    } = browserEnvironment;
    ...
    })} dir={textDirection} onClick={() => setMouseUserState(true)} onKeyDown={e => {
      if (e.keyCode === 9) {
        ...
        <Sidebar sidebarOpen={sidebarIsOpen} sidebarShouldClose={sidebarShouldClose} hideSidebar={this.props.hideSidebar} transitionName={sidebarTransitionName} type={sidebarType} sidebarProps={sidebar.props} />
        <NetworkDropdown provider={provider} frequentRpcListDetail={frequentRpcListDetail} />
        <AccountMenu />
