...
import { DEX_DISPLAY_NAME, MAINNET_DISPLAY_NAME, NETWORK_TO_NAME_MAP, NETWORK_TYPE_TO_ID_MAP } from '@shared/constants/network';
...
export default class EnsInput extends Component {
  ...
  onPaste = event => {
    event.clipboardData.items[0].getAsString(text => {
      ...
      if (!isBurnAddress(input) && isValidHexAddress(input, {
  ...
  onChange = ({
    ...
    if (isValidDomainName(input)) {
      ...
      if (onValidAddressTyped && !isBurnAddress(input) && isValidHexAddress(input, {
        mixedCaseUseChecksum: true
      })) {
  ...
  render() {
    ...
    Object.keys(NETWORK_TYPE_TO_ID_MAP).map(key => {
      if (toBnString(NETWORK_TYPE_TO_ID_MAP[key].chainId) == bnStrChainId) {
        placeholder = MAINNET_DISPLAY_NAME.toUpperCase() == NETWORK_TO_NAME_MAP[key].toUpperCase() ? t('recipientAddressPlaceholder', ['ENS']) : DEX_DISPLAY_NAME.toUpperCase() == NETWORK_TO_NAME_MAP[key].toUpperCase() ? t('recipientAddressPlaceholder', ['DNS']) : t('recipientAddressPlaceholder2');
      }
    });
    const hasSelectedAddress = Boolean(selectedAddress);
    return <div className={classnames('ens-input', className)}>
        ...
        'ens-input__wrapper--valid': hasSelectedAddress
          ...
          <div className={classnames('ens-input__wrapper__status-icon', {
          ...
          {hasSelectedAddress ? <>
              <div className="ens-input__wrapper__input ens-input__wrapper__input--selected">
                ...
                <div>
                  ...
                  {selectedName && <div className="ens-input__selected-input__subtitle">
                      {ellipsify(selectedAddress, 14, 6)}
                    </div>}
                </div>
              </div>
              <div className="ens-input__wrapper__action-icon ens-input__wrapper__action-icon--erase" onClick={this.props.onReset} />
            </> : <>
              <input className="ens-input__wrapper__input" type="text" dir="auto" placeholder={placeholder} onChange={this.onChange} onPaste={this.onPaste} value={selectedAddress || userInput} autoFocus data-testid="ens-input" />
              <button className={classnames('ens-input__wrapper__action-icon', {
