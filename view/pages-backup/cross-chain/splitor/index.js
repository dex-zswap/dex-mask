...
const CrossChainSplitor = () => {
  ...
  const fromChainProviderType = useMemo(() => {
    ...
    for (let i = 0, {
      length
    } = types, providerType, providerInfo; i < length; i++) {
    ...
    checkTokenBridge({
      ...
      if (res.c === 200) {
        const target = res.d.find(({
          ...
          token_address
        }) => target_meta_chain_id === toBnString(crossInfo.fromChain) && token_address === crossInfo.coinAddress);
  ...
  return <div className={classnames(['cross-chain__splitor', isDifferentChain && 'diffrent-chain'])}>
      <div>
        {isDifferentChain ? <div className="bridge">{t('dexBridge')}</div> : null}
        {reverseCross ? <div className="reverse-cross" onClick={reverseTokenAndChain}>
