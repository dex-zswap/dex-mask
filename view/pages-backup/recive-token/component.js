...
class ReciveToken extends React.Component {
  ...
  async chainChange(type, chainType, chainId, isRpc, chainInfo) {
    ...
    const { currentChainId } = this.props;
        ...
        <div>
          <div className="recive-token__from">
            <CurrentToken
              ...
              diameter={40}
              currentChainId={from.chainId}
              onChange={(token) => this.tokenChange(token)}
              selectable
            >
              <AccountSwitcher
                needOutSide={true}
                address={from.account}
                onChange={(account) => this.accountChange('from', account)}
              />
            </CurrentToken>
            <ChainSwitcher
              currentChainId={from.chainId}
              onChange={(type, chainId, isRpc, chainInfo) =>
                this.chainChange('from', type, chainId, isRpc, chainInfo)
              }
            />
          </div>
          <div
          ...
          <div className="recive-token__to">
            <div className="recive-token__to-top">
              <CurrentToken
                coinAddress={coinAddress}
                diameter={40}
                currentChainId={to.chainId}
              >
                <AccountSwitcher
                  address={to.account}
                  onChange={(account) => this.accountChange('to', account)}
                />
              </CurrentToken>
              <ChainSwitcher
                currentChainId={to.chainId}
                onChange={(type, chainId, isRpc, chainInfo) =>
                  this.chainChange('to', type, chainId, isRpc, chainInfo)
                }
              />
            </div>
            {to.account && (
              <div className="recive-token__qr-code">
                <QrView
                  hiddenAddress={true}
                  cellWidth={5}
                  darkColor="#fff"
