...
import { Link, matchPath, Redirect, Route, Switch, useLocation } from 'react-router-dom';
...
export default function Trade() {
  ...
  return <div className="trade-component__wrapper">
      <PageTitle title={t('swap')} subTitle={activeInfo.swap ? t('zswap') : t('liquidity')} />
      <div className="trade-component__tabs">
        <div className={classnames(['trade-component__tab', activeInfo.swap && 'active'])}>
          <Link to={ZSWAP_ROUTE}>{t('zswap')}</Link>
        </div>
        <div className={classnames(['trade-component__tab', activeInfo.liquidity && 'active'])}>
