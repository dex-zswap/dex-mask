...
import { getCurrentChainId, getDexMaskAccounts, getDexMaskAccountsConnected, getRpcPrefsForCurrentProvider } from '@view/selectors';
...
class ConnectHardwareForm extends Component {
    ...
    this.props.connectHardware(device, page, hdPath).then(accounts => {
      if (accounts.length) {
        ...
        const newAccounts = accounts.map(account => {
          ...
          const balanceValue = this.props.accounts[normalizedAddress]?.balance || null;
  ...
  renderError() {
    if (this.state.error === U2F_ERROR) {
      return <p className="hw-connect__error">
          {this.context.t('troubleConnectingToWallet', [this.state.device, // eslint-disable-next-line react/jsx-key
        <a href="https://metamask.zendesk.com/hc/en-us/articles/360020394612-How-to-connect-a-Trezor-or-Ledger-Hardware-Wallet" key="hardware-connection-guide" target="_blank" rel="noopener noreferrer" className="hw-connect__link" style={{
          marginLeft: '5px',
          marginRight: '5px'
        }}>
