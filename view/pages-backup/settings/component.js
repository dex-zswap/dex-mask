...
class SettingsPage extends PureComponent {
  ...
  render() {
    ...
    return <div className={classnames('main-container settings-page', {
      'settings-page--selected': currentPath !== SETTINGS_ROUTE
    })}>
        <PageTitle title={NETWORKS_FORM_ROUTE == pathname ? this.context.t('custom') : this.context.t('manage')} subTitle={this.getTitleText()} />
  ...
  renderTitle() {
    return <div className="settings-page__header__title">{this.getTitleText()}</div>;
  }
  ...
  renderSubHeader() {
    ...
    return !currentPath.startsWith(NETWORKS_ROUTE) && <div className="settings-page__subheader">
        ...
        'settings-page__subheader--link': initialBreadCrumbRoute
          ...
          {breadCrumbTextKey && <div className="settings-page__subheader--break">
              <span>{' > '}</span>
              {t(breadCrumbTextKey)}
            </div>}
          {isAddressEntryPage && <div className="settings-page__subheader--break">
              <span>{' > '}</span>
              {addressName}
            </div>}
        </div>;
  ...
  renderContent() {
    return <Switch>
        ...
        <Route exact path={`${CONTACT_EDIT_ROUTE}/:id`} component={ContactListTab} />
        <Route exact path={`${CONTACT_VIEW_ROUTE}/:id`} component={ContactListTab} />
