...
import { fetchQuotesAndSetQuoteState, getApproveTxParams, getFetchParams, getUsedQuote, getUsedSwapsGasPrice, navigateBackToBuildQuote, prepareForRetryGetQuotes, prepareToLeaveSwaps } from '@reducer/swaps/swaps';
import { CHAINID_EXPLORE_MAP } from '@shared/constants/network';
import { CONTRACT_DATA_DISABLED_ERROR, ERROR_FETCHING_QUOTES, OFFLINE_FOR_MAINTENANCE, QUOTES_EXPIRED_ERROR, QUOTES_NOT_AVAILABLE_ERROR, SWAPS_CHAINID_DEFAULT_BLOCK_EXPLORER_URL_MAP, SWAP_FAILED_ERROR } from '@shared/constants/swaps';
...
import { getCurrentChainId, getCurrentCurrency, getHardwareWalletType, getRpcPrefsForCurrentProvider, getUSDConversionRate, isHardwareWallet } from '@view/selectors';
...
export default function AwaitingSwap({
  ...
  const baseNetworkUrl = rpcPrefs.blockExplorerUrl ?? SWAPS_CHAINID_DEFAULT_BLOCK_EXPLORER_URL_MAP[chainId] ?? null;
  ...
  if (!blockExplorerUrl && CHAINID_EXPLORE_MAP[chainId]) {
    blockExplorerUrl = createCustomExplorerLink(txHash, CHAINID_EXPLORE_MAP[chainId]);
  }

  const isCustomBlockExplorerUrl = Boolean(SWAPS_CHAINID_DEFAULT_BLOCK_EXPLORER_URL_MAP[chainId] || rpcPrefs.blockExplorerUrl);
  let headerText;
  let statusImage;
  let descriptionText;
  let submitText;
  ...
  if (errorKey === OFFLINE_FOR_MAINTENANCE) {
    ...
    descriptionText = t('swapFailedErrorDescriptionWithSupportLink', [<a className="awaiting-swap__support-link" key="awaiting-swap-support-link" href="https://support.metamask.io" target="_blank" rel="noopener noreferrer">
        support.metamask.io
      </a>]);
    submitText = t('tryAgain');
    statusImage = <SwapFailureIcon />;
    content = blockExplorerUrl && <ViewOnEtherScanLink txHash={txHash} blockExplorerUrl={blockExplorerUrl} isCustomBlockExplorerUrl={isCustomBlockExplorerUrl} />;
    ...
    descriptionText = t('swapOnceTransactionHasProcess', [<span key="swapOnceTransactionHasProcess-1" className="awaiting-swap__amount-and-symbol">
    ...
    submitText = t('close');
    descriptionText = t('swapTokenAvailable', [<span key="swapTokenAvailable-2" className="awaiting-swap__amount-and-symbol">
  ...
  return <div className="awaiting-swap">
      <div className="awaiting-swap__content">
        {!(swapComplete || errorKey) && <Mascot animationEventEmitter={animationEventEmitter.current} width="90" height="90" />}
      ...
      } else if (isSwapsDefaultTokenSymbol(destinationTokenInfo?.symbol, chainId) || swapComplete) {
        history.push(DEFAULT_ROUTE);
      } else {
        history.push(`${ASSET_ROUTE}/${destinationTokenInfo?.address}`);
      }
    }} onCancel={async () => await dispatch(navigateBackToBuildQuote(history))} submitText={submitText} disabled={submittingSwap} hideCancel={errorKey !== QUOTES_EXPIRED_ERROR} />
    </div>;
}
AwaitingSwap.propTypes = {
  ...
  errorKey: PropTypes.oneOf([QUOTES_EXPIRED_ERROR, SWAP_FAILED_ERROR, ERROR_FETCHING_QUOTES, QUOTES_NOT_AVAILABLE_ERROR, OFFLINE_FOR_MAINTENANCE, CONTRACT_DATA_DISABLED_ERROR]),
