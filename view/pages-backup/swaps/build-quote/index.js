...
import { fetchQuotesAndSetQuoteState, getBalanceError, getFetchParams, getFromToken, getTopAssets, getToToken, setSwapsFromToken, setSwapToToken } from '@reducer/swaps/swaps';
import { CHAINID_EXPLORE_MAP } from '@shared/constants/network';
import { SWAPS_CHAINID_DEFAULT_BLOCK_EXPLORER_URL_MAP, SWAPS_CHAINID_DEFAULT_TOKEN_MAP } from '@shared/constants/swaps';
...
import { getRenderableTokenData, useTokensToSearch } from '@view/hooks/useTokensToSearch';
import { useTokenTracker } from '@view/hooks/useTokenTracker';
import { getCurrentChainId, getCurrentCurrency, getRpcPrefsForCurrentProvider, getSwapsDefaultToken, getTokenExchangeRates } from '@view/selectors';
...
export default function BuildQuote({
  ...
  const fetchParamsFromToken = isSwapsDefaultTokenSymbol(sourceTokenInfo?.symbol, chainId) ? defaultSwapsToken : sourceTokenInfo;
  const {
    loading,
    tokensWithBalances
  } = useTokenTracker(tokens); // If the fromToken was set in a call to `onFromSelect` (see below), and that from token has a balance
  ...
  const fromTokenArray = !isSwapsDefaultTokenSymbol(fromToken?.symbol, chainId) && fromToken?.balance ? [fromToken] : [];
  ...
  const selectedFromToken = getRenderableTokenData(fromToken || fetchParamsFromToken, tokenConversionRates, conversionRate, currentCurrency, chainId);
  ...
  const toTokenIsNotDefault = selectedToToken?.address && !isSwapsDefaultTokenAddress(selectedToToken?.address, chainId);
  const occurrences = Number(selectedToToken?.occurances || selectedToToken?.occurrences || 0);
  const {
  ...
  const swapFromTokenFiatValue = useTokenFiatAmount(fromTokenAddress, inputValue || 0, fromTokenSymbol, {
    showFiat: true
  }, true);
  const swapFromEthFiatValue = useEthFiatAmount(inputValue || 0, {
    showFiat: true
  }, true);
  const swapFromFiatValue = isSwapsDefaultTokenSymbol(fromTokenSymbol, chainId) ? swapFromEthFiatValue : swapFromTokenFiatValue;
  ...
  const onFromSelect = token => {
    ...
    if (token?.address && !memoizedUsersTokens.find(usersToken => usersToken.address === token.address)) {
      fetchTokenBalance(token.address, selectedAccountAddress).then(fetchedBalance => {
  ...
  const blockExplorerTokenLink = getTokenTrackerLink(selectedToToken.address, chainId, null, // no networkId
  null, // no holderAddress
  {
    blockExplorerUrl: rpcPrefs.blockExplorerUrl ?? SWAPS_CHAINID_DEFAULT_BLOCK_EXPLORER_URL_MAP[chainId] ?? CHAINID_EXPLORE_MAP[chainId] ?? null
  });
  const blockExplorerLabel = rpcPrefs.blockExplorerUrl ? new URL(blockExplorerTokenLink).hostname : t('etherscan');
  const {
  ...
  useEffect(() => {
    const notDefault = !isSwapsDefaultTokenAddress(tokensWithBalancesFromToken?.address, chainId);
    ...
    if (notDefault && addressesAreTheSame && balanceHasChanged) {
      dispatch(setSwapsFromToken({ ...fromToken,
  ...
  useEffect(() => {
    if (isSwapsDefaultTokenAddress(fromToken?.address, chainId) && fromToken?.balance !== hexToDecimal(ethBalance)) {
      dispatch(setSwapsFromToken({ ...fromToken,
  ...
  const BlockExplorerLink = () => {
    return <a className="build-quote__token-etherscan-link build-quote__underline" key="build-quote-etherscan-link" onClick={() => {
  ...
  if (blockExplorerTokenLink) {
    if (occurrences === 1) {
      tokenVerificationDescription = t('verifyThisTokenOn', [<BlockExplorerLink key="block-explorer-link" />]);
    } else if (occurrences === 0) {
      tokenVerificationDescription = t('verifyThisUnconfirmedTokenOn', [<BlockExplorerLink key="block-explorer-link" />]);
    }
  }

  const swapYourTokenBalance = t('swapYourTokenBalance', [fromTokenString || '0', fromTokenSymbol || SWAPS_CHAINID_DEFAULT_TOKEN_MAP[chainId]?.symbol || '']);
  return <div className="build-quote">
      <div className="build-quote__content">
        <div className="build-quote__dropdown-input-pair-header">
          <div className="build-quote__input-label">{t('swapSwapFrom')}</div>
          {!isSwapsDefaultTokenSymbol(fromTokenSymbol, chainId) && <div className="build-quote__max-button" onClick={() => onInputChange(fromTokenBalance || '0', fromTokenBalance)}>
              {t('max')}
            </div>}
        </div>
        <DropdownInputPair onSelect={onFromSelect} itemsToSearch={tokensToSearch} onInputChange={value => {
        onInputChange(value, fromTokenBalance);
      }} inputValue={inputValue} leftValue={inputValue && swapFromFiatValue} selectedItem={selectedFromToken} maxListItems={30} loading={loading && (!tokensToSearch?.length || !topAssets || !Object.keys(topAssets).length)} selectPlaceHolderText={t('swapSelect')} hideItemIf={item => item.address === selectedToToken?.address} listContainerClassName="build-quote__open-dropdown" autoFocus />
        <div className={classnames('build-quote__balance-message', {
        'build-quote__balance-message--error': balanceError || tokenFromError
      })}>
          {!tokenFromError && !balanceError && fromTokenSymbol && swapYourTokenBalance}
          {!tokenFromError && balanceError && fromTokenSymbol && <div className="build-quite__insufficient-funds">
        ...
        <div className="build-quote__swap-arrows-row">
          <button className="build-quote__swap-arrows" onClick={() => {
        ...
        <div className="dropdown-input-pair dropdown-input-pair__to">
          <DropdownSearchList startingItem={selectedToToken} itemsToSearch={tokensToSearch} searchPlaceholderText={t('swapSearchForAToken')} fuseSearchKeys={fuseSearchKeys} selectPlaceHolderText={t('swapSelectAToken')} maxListItems={30} onSelect={onToSelect} loading={loading && (!tokensToSearch?.length || !topAssets || !Object.keys(topAssets).length)} externallySelectedItem={selectedToToken} hideItemIf={hideDropdownItemIf} listContainerClassName="build-quote__open-to-dropdown" hideRightLabels defaultToAll shouldSearchForImports />
        </div>
        {toTokenIsNotDefault && (occurrences < 2 ? <ActionableMessage type={occurrences === 1 ? 'warning' : 'danger'} message={<div className="build-quote__token-verification-warning-message">
                ...
                </div>} primaryAction={verificationClicked ? null : {
        label: t('continue'),
        onClick: () => setVerificationClicked(true)
      }} withRightButton infoTooltipText={blockExplorerTokenLink && t('swapVerifyTokenExplanation', [blockExplorerLabel])} /> : <div className="build-quote__token-message">
              <span className="build-quote__bold" key="token-verification-bold-text">
                {t('swapTokenVerificationSources', [occurrences])}
              </span>
              {blockExplorerTokenLink && <>
                  {t('swapTokenVerificationMessage', [<a className="build-quote__token-etherscan-link" key="build-quote-etherscan-link" onClick={() => {
            ...
            global.platform.openTab({
              url: blockExplorerTokenLink
                  ...
                  <InfoTooltip position="top" contentText={t('swapVerifyTokenExplanation', [blockExplorerLabel])} containerClassName="build-quote__token-tooltip-container" key="token-verification-info-tooltip" />
                </>}
            </div>)}
        <div className="build-quote__slippage-buttons-container">
          <SlippageButtons onSelect={newSlippage => {
          setMaxSlippage(newSlippage);
        }} maxAllowedSlippage={MAX_ALLOWED_SLIPPAGE} currentSlippage={maxSlippage} />
        </div>
      </div>
      <SwapsFooter onSubmit={() => {
      dispatch(fetchQuotesAndSetQuoteState(history, inputValue, maxSlippage, metaMetricsEvent));
    }} submitText={t('swapReviewSwap')} disabled={tokenFromError || !isFeatureFlagLoaded || !Number(inputValue) || !selectedToToken?.address || Number(maxSlippage) < 0 || Number(maxSlippage) > MAX_ALLOWED_SLIPPAGE || toTokenIsNotDefault && occurrences < 2 && !verificationClicked} hideCancel showTermsOfService />
    </div>;
}
