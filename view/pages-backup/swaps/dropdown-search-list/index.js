...
import { getCurrentChainId, getHardwareWalletType, getRpcPrefsForCurrentProvider, isHardwareWallet } from '@selectors/selectors';
...
export default function DropdownSearchList({
  ...
  const blockExplorerLink = rpcPrefs.blockExplorerUrl ?? SWAPS_CHAINID_DEFAULT_BLOCK_EXPLORER_URL_MAP[chainId] ?? null;
  const blockExplorerLabel = rpcPrefs.blockExplorerUrl ? new URL(blockExplorerLink).hostname : t('etherscan');
  ...
  return <div className={classnames('dropdown-search-list', className)} onClick={onClickSelector} onKeyUp={onKeyUp} tabIndex="0">
      {tokenForImport && isImportTokenModalOpen && <ImportToken {...importTokenProps} />}
      {!isOpen && <div className={classnames('dropdown-search-list__selector-closed-container', selectorClosedClassName)}>
          <div className="dropdown-search-list__selector-closed">
            ...
            <div className="dropdown-search-list__labels">
              <div className="dropdown-search-list__item-labels">
                <span className={classnames('dropdown-search-list__closed-primary-label', {
      ...
      {isOpen && <>
          <SearchableItemList itemsToSearch={loading ? [] : itemsToSearch} Placeholder={({
        searchQuery
      }) => loading ? <div className="dropdown-search-list__loading-item">
                  <PulseLoader />
                ...
                </div> : <div className="dropdown-search-list__placeholder">
                  {t('swapBuildQuotePlaceHolderText', [searchQuery])}
                  <div tabIndex="0" className="searchable-item-list__item searchable-item-list__item--add-token" key="searchable-item-list-item-last">
                    <ActionableMessage message={blockExplorerLink && t('addCustomTokenByContractAddress', [<a key="dropdown-search-list__etherscan-link" onClick={() => {
            ...
            global.platform.openTab({
              url: blockExplorerLink
                ...
                </div>} searchPlaceholderText={t('swapSearchForAToken')} fuseSearchKeys={fuseSearchKeys} defaultToAll={defaultToAll} onClickItem={onClickItem} onOpenImportTokenModalClick={onOpenImportTokenModalClick} maxListItems={maxListItems} className={classnames('dropdown-search-list__token-container', searchListClassName, {
        'dropdown-search-list--open': isOpen
      })} hideRightLabels={hideRightLabels} hideItemIf={hideItemIf} listContainerClassName={listContainerClassName} shouldSearchForImports={shouldSearchForImports} />
          <div className="dropdown-search-list__close-area" onClick={event => {
