...
import CountdownTimer from '@pages/swaps/countdown-timer';
import FeeCard from '@pages/swaps/fee-card';
import MainQuoteSummary from '@pages/swaps/main-quote-summary';
import SelectQuotePopover from '@pages/swaps/select-quote-popover';
import SwapsFooter from '@pages/swaps/swaps-footer';
import { getRenderableNetworkFeesForQuote, quotesToRenderableData } from '@pages/swaps/swaps.util';
import { getNativeCurrency, getTokens } from '@reducer/dexmask/dexmask';
import { FALLBACK_GAS_MULTIPLIER, getApproveTxParams, getBackgroundSwapRouteState, getBalanceError, getCustomMaxFeePerGas, getCustomMaxPriorityFeePerGas, getCustomSwapsGas, getDestinationTokenInfo, getFetchParams, getQuotes, getQuotesLastFetched, getSelectedQuote, getSwapsQuoteRefreshTime, getSwapsUserFeeLevel, getTopQuote, getUsedSwapsGasPrice, navigateBackToBuildQuote, setBalanceError, signAndSendTransactions, swapsQuoteSelected } from '@reducer/swaps/swaps';
import { EDIT_GAS_MODES } from '@shared/constants/gas';
import { QUOTES_EXPIRED_ERROR } from '@shared/constants/swaps';
import { I18nContext } from '@view/contexts/i18n';
import { MetaMetricsContext } from '@view/contexts/metametrics.new';
import { ASSET_ROUTE, AWAITING_SWAP_ROUTE, BUILD_QUOTE_ROUTE, DEFAULT_ROUTE, SWAPS_ERROR_ROUTE } from '@view/helpers/constants/routes';
import { toPrecisionWithoutTrailingZeros } from '@view/helpers/utils';
import { addHexes, decGWEIToHexWEI, decimalToHex, getValueFromWeiHex, hexToDecimal, hexWEIToDecGWEI } from '@view/helpers/utils/conversions.util';
import { calcTokenAmount, calcTokenValue, getTokenValueParam } from '@view/helpers/utils/token-util';
...
import { checkNetworkAndAccountSupports1559, conversionRateSelector, getCurrentChainId, getCurrentCurrency, getHardwareWalletType, getSelectedAccount, getSwapsDefaultToken, getTokenExchangeRates, isHardwareWallet } from '@view/selectors';
import { safeRefetchQuotes, setCustomApproveTxData, setSwapsErrorKey, showModal } from '@view/store/actions';
...
export default function ViewQuote() {
  ...
  const [acknowledgedPriceDifference, setAcknowledgedPriceDifference] = useState(false);
  ...
  const {
    balance: ethBalance
  } = useSelector(getSelectedAccount);
  const conversionRate = useSelector(conversionRateSelector);
  const currentCurrency = useSelector(getCurrentCurrency);
  const swapsTokens = useSelector(getTokens);
  const networkAndAccountSupports1559 = useSelector(checkNetworkAndAccountSupports1559);
  ...
  const usedGasLimitWithMultiplier = new BigNumber(gasLimitForMax, 16).times(usedQuote?.gasMultiplier || FALLBACK_GAS_MULTIPLIER, 10).round(0).toString(16);
  const nonCustomMaxGasLimit = usedQuote?.gasEstimate ? usedGasLimitWithMultiplier : `0x${decimalToHex(usedQuote?.maxGas || 0)}`;
  ...
  if (networkAndAccountSupports1559) {
    ...
    maxPriorityFeePerGas = customMaxPriorityFeePerGas || decGWEIToHexWEI(suggestedMaxPriorityFeePerGas);
    baseAndPriorityFeePerGas = addHexes(decGWEIToHexWEI(estimatedBaseFee), maxPriorityFeePerGas);
  }

  const gasTotalInWeiHex = calcGasTotal(maxGasLimit, networkAndAccountSupports1559 ? maxFeePerGas : gasPrice);
  const {
    tokensWithBalances
  ...
  const approveAmount = approveValue && selectedFromToken?.decimals !== undefined && calcTokenAmount(approveValue, selectedFromToken.decimals).toFixed(9);
  const approveGas = approveTxParams?.gas;
  const renderablePopoverData = useMemo(() => {
    return quotesToRenderableData(quotes, networkAndAccountSupports1559 ? baseAndPriorityFeePerGas : gasPrice, conversionRate, currentCurrency, approveGas, memoizedTokenConversionRates, chainId);
  ...
  const tokenBalanceNeeded = insufficientTokens ? toPrecisionWithoutTrailingZeros(calcTokenAmount(tokenCost, selectedFromToken.decimals).minus(tokenBalance).toString(10), 6) : null;
  const ethBalanceNeeded = insufficientEth ? toPrecisionWithoutTrailingZeros(ethCost.minus(ethBalance, 16).div('1000000000000000000', 10).toString(10), 6) : null;
  ...
  }, [sourceTokenSymbol, sourceTokenValue, destinationTokenSymbol, destinationTokenValue, fetchParams, topQuote, numberOfQuotes, feeInFiat]);
  ...
  const onFeeCardTokenApprovalClick = () => {
    dispatch(showModal({
      ...
      requiredMinimum: calcTokenAmount(usedQuote.sourceAmount, selectedFromToken.decimals)
    }));
  };

  const nonGasFeeIsPositive = new BigNumber(nonGasFee, 16).gt(0);
  const approveGasTotal = calcGasTotal(approveGas || '0x0', networkAndAccountSupports1559 ? baseAndPriorityFeePerGas : gasPrice);
  const extraNetworkFeeTotalInHexWEI = new BigNumber(nonGasFee, 16).plus(approveGasTotal, 16).toString(16);
  ...
  const onFeeCardMaxRowClick = () => {
    networkAndAccountSupports1559 ? setShowEditGasPopover(true) : dispatch(showModal({
  ...
  const tokenApprovalTextComponent = <span key="swaps-view-quote-approve-symbol-1" className="view-quote__bold">
      {sourceTokenSymbol}
    </span>;
  const actionableBalanceErrorMessage = tokenBalanceUnavailable ? t('swapTokenBalanceUnavailable', [sourceTokenSymbol]) : t('swapApproveNeedMoreTokens', [<span key="swapApproveNeedMoreTokens-1" className="view-quote__bold">
          {tokenBalanceNeeded || ethBalanceNeeded}
        </span>, tokenBalanceNeeded && !(sourceTokenSymbol === defaultSwapsToken.symbol) ? sourceTokenSymbol : defaultSwapsToken.symbol]); // Price difference warning
  ...
  const priceSlippageFromSource = useEthFiatAmount(usedQuote?.priceSlippage?.sourceAmountInETH || 0, {
    showFiat: true
  });
  const priceSlippageFromDestination = useEthFiatAmount(usedQuote?.priceSlippage?.destinationAmountInETH || 0, {
    showFiat: true
  }); // We cannot present fiat value if there is a calculation error or no slippage
  // from source or destination

  const priceSlippageUnknownFiatValue = !priceSlippageFromSource || !priceSlippageFromDestination || usedQuote?.priceSlippage?.calculationError;
  let priceDifferencePercentage = 0;

  if (usedQuote?.priceSlippage?.ratio) {
    priceDifferencePercentage = parseFloat(new BigNumber(usedQuote.priceSlippage.ratio, 10).minus(1, 10).times(100, 10).toFixed(2), 10);
  }
  ...
  return <div className="view-quote">
      <div className={classnames('view-quote__content', {
      'view-quote__content_modal': disableSubmissionDueToPriceWarning
    })}>
        {selectQuotePopoverShown && <SelectQuotePopover quoteDataRows={renderablePopoverData} onClose={() => setSelectQuotePopoverShown(false)} onSubmit={aggId => dispatch(swapsQuoteSelected(aggId))} swapToSymbol={destinationTokenSymbol} initialAggId={usedQuote.aggregator} />}

        {showEditGasPopover && networkAndAccountSupports1559 && <EditGasPopover transaction={{
        userFeeLevel: swapsUserFeeLevel || 'high',
        txParams: {
          maxFeePerGas,
      ...
      }} minimumGasLimit={usedGasLimit} defaultEstimateToUse="high" mode={EDIT_GAS_MODES.SWAPS} confirmButtonText={t('submit')} onClose={onCloseEditGasPopover} />}

        <div className={classnames('view-quote__warning-wrapper', {
        'view-quote__warning-wrapper--thin': !isShowingWarning
      })}>
          {viewQuotePriceDifferenceComponent}
          {(showInsufficientWarning || tokenBalanceUnavailable) && <ActionableMessage message={actionableBalanceErrorMessage} onClose={() => setWarningHidden(true)} />}
        </div>
        <div className="view-quote__countdown-timer-container">
          <CountdownTimer timeStarted={quotesLastFetched} warningTime="0:30" infoTooltipLabelKey="swapQuotesAreRefreshed" labelKey="swapNewQuoteIn" />
        </div>
        <MainQuoteSummary sourceValue={calcTokenValue(sourceTokenValue, sourceTokenDecimals)} sourceDecimals={sourceTokenDecimals} sourceSymbol={sourceTokenSymbol} destinationValue={calcTokenValue(destinationTokenValue, destinationTokenDecimals)} destinationDecimals={destinationTokenDecimals} destinationSymbol={destinationTokenSymbol} sourceIconUrl={sourceTokenIconUrl} destinationIconUrl={destinationIconUrl} />
        ...
        'view-quote__fee-card-container--three-rows': approveTxParams && (!balanceError || warningHidden)
          ...
          fee: feeInFiat,
          maxFee: maxFeeInFiat
        }} onFeeCardMaxRowClick={onFeeCardMaxRowClick} hideTokenApprovalRow={!approveTxParams || balanceError && !warningHidden} tokenApprovalTextComponent={tokenApprovalTextComponent} tokenApprovalSourceTokenSymbol={sourceTokenSymbol} onTokenApprovalClick={onFeeCardTokenApprovalClick} metaMaskFee={String(metaMaskFee)} isBestQuote={isBestQuote} numberOfQuotes={Object.values(quotes).length} onQuotesClick={() => {
          setSelectQuotePopoverShown(true);
        }} tokenConversionRate={destinationTokenSymbol === defaultSwapsToken.symbol ? 1 : memoizedTokenConversionRates[destinationToken.address]} chainId={chainId} networkAndAccountSupports1559={networkAndAccountSupports1559} maxPriorityFeePerGasDecGWEI={hexWEIToDecGWEI(maxPriorityFeePerGas)} maxFeePerGasDecGWEI={hexWEIToDecGWEI(maxFeePerGas)} />
        </div>
      </div>
      <SwapsFooter onSubmit={() => {
