...
export default class RevealSeedPhraseComponent extends PureComponent {
  ...
  renderPhrase = () => {
    ...
    return <div className="reveal-seed-phrase__word-display">
        <div className="reveal-seed-phrase__word-column">
          {seedPhrase.slice(0, 6).map((str, index) => <div className="word-item" key={str}>
              {index + 1}.<span className="word-text">{str}</span>
            </div>)}
        </div>
        <div className="reveal-seed-phrase__word-column">
          {seedPhrase.slice(6).map((str, index) => <div className="word-item" key={str}>
    ...
    return <div>
        ...
        <div className="first-time-flow__form">
          ...
          <div className="first-time-flow__account-password-btn">
            <Button type="primary" className="first-time-flow__button" onClick={this.handleNext} rightArrow={true}>
              {t('next')}
            </Button>
          </div>
        </div>
