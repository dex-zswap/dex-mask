...
export default function AdvancedTab({
  ...
  return <div className="setting-advanced-wrap base-width">
      ...
      <div className="setting-item">
        ...
        <Button type="warning" onClick={event => {
      ...
      <div className="setting-item">
        ...
        <Switch value={advancedInlineGas} onChange={() => dispatch(setFeatureFlag('advancedInlineGas', !advancedInlineGas))} />
      </div>
      <div className="setting-item">
        <div className="setting-label">{t('showHexData')}</div>
        <div className="setting-value">{t('showHexDataDescription')}</div>
        <Switch value={sendHexData} onChange={() => dispatch(setFeatureFlag('sendHexData', !sendHexData))} />
      </div>
      <div className="setting-item">
        ...
        <Switch value={showFiatInTestnets} onChange={() => dispatch(setShowFiatConversionOnTestnetsPreference(!showFiatInTestnets))} />
      </div>
      <div className="setting-item">
        <div className="setting-label">{t('nonceField')}</div>
        <div className="setting-value">{t('nonceFieldDescription')}</div>
        <Switch value={useNonceField} onChange={() => dispatch(setUseNonceField(!useNonceField))} />
      </div>
      <div className="setting-item">
        ...
        <div className="setting-input-btn-wrap">
          <TextField type="number" id="autoTimeout" placeholder="5" value={autoLockTimeLimit} defaultValue={autoLockTimeLimit} onChange={e => handleLockChange(e.target.value)} error={lockTimeError} fullWidth margin="dense" min={0} />
          <Button type="primary" className="settings-tab__rpc-save-button" disabled={lockTimeError !== ''} onClick={() => dispatch(setAutoLockTimeLimit(autoLockTimeLimit))}>
            {t('save')}
          </Button>
        </div>
      </div>
      <div className="setting-item">
        ...
        <div className="setting-value">
          {threeBoxDisabled ? t('syncWithThreeBoxDisabled') : t('syncWithThreeBoxDescription')}
        </div>
        <Switch value={allowed} onChange={() => {
        if (!threeBoxDisabled) {
          if (!allowed) {
      ...
      <div className="setting-item">
        ...
        <div className="setting-input-btn-wrap">
          <TextField type="text" value={ipfsGateway} onChange={e => handleIpfsGatewayChange(e.target.value)} error={ipfsGatewayError} fullWidth margin="dense" />
          <Button type="primary" className="settings-tab__rpc-save-button" disabled={Boolean(ipfsGatewayError)} onClick={() => {
          const {
            host
          } = new URL(addUrlProtocolPrefix(ipfsGateway));
      ...
      <div className="setting-item">
        ...
        <Switch value={useLedgerLive} onChange={() => dispatch(setLedgerLivePreference(!useLedgerLive))} disabled={getPlatform() === PLATFORM_FIREFOX} />
      </div>
      <div className="setting-item">
        ...
        <Switch value={dismissSeedBackUpReminder} onChange={() => dispatch(setDismissSeedBackUpReminder(!dismissSeedBackUpReminder))} />
      </div>
    </div>;
}