...
import { getFetchParams, getQuotesFetchStartTime, navigateBackToBuildQuote } from '@reducer/swaps/swaps';
...
export default function LoadingSwapsQuotes({
  ...
  const _aggregatorLocationMap = aggregatorNames.reduce((nameLocationMap, name, index) => ({ ...nameLocationMap,
  ...
  return <div className="loading-swaps-quotes">
      <div className="loading-swaps-quotes__content">
        <>
          ...
          <div className="loading-swaps-quotes__loading-bar-container">
            <div className="loading-swaps-quotes__loading-bar" style={{
        ...
        <div className="loading-swaps-quotes__animation">
          ...
          <div className="loading-swaps-quotes__mascot-container" ref={mascotContainer}>
            <Mascot animationEventEmitter={animationEventEmitter.current} width="90" height="90" followMouse={false} lookAtTarget={getMascotTarget(aggregatorNames[quoteCount], midPointTarget, aggregatorLocationMap)} />
          </div>
          {currentMascotContainer && midPointTarget && aggregatorNames.map(aggName => <div className={classnames('loading-swaps-quotes__logo', {
          ...
          left: aggregatorLocationMap[aggName]?.x + midPointTarget?.x ?? 0
                ...
                <AggregatorLogo name={aggregatorMetadata[aggName]?.title} icon={aggregatorMetadata[aggName]?.icon} color={aggregatorMetadata[aggName]?.color} />
              </div>)}
        </div>
      </div>
      <SwapsFooter submitText={t('back')} onSubmit={async () => {
