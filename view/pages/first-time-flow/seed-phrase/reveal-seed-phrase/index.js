...
export default function RevealSeedPhrase({
  ...
  return <div className="reveal-seed-phrase dex-page-container space-between base-width">
      <div>
        <Logo plain />
        ...
        <div className="first-time-flow__form">
          ...
          return <div className={classnames('reveal-seed-phrase__word-column', cIndex ? 'right' : 'left')} key={phrases.join('')}>
                  {phrases.map((phrase, index) => <div className="word-item" key={phrase}>
      ...
      <div className="first-time-flow__account-password-btn">
        <Button type="primary" className="first-time-flow__button" onClick={handleNext}>
          {t('next')}
        </Button>
      </div>
    </div>;
