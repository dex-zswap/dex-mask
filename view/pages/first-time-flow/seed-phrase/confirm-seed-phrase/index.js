...
export default function ConfirmSeedPhrase({
  ...
  return <div className="confirm-seed-phrase dex-page-container space-between base-width">
      <div>
        <Logo plain />
        ...
        <div className="seed-phrase-display">
          <div className="first-time-flow__form">
            ...
            return <div className={classnames('confirm-seed-phrase__word-column', cIndex ? 'right' : 'left')} key={phrases.join('')}>
                    {phrases.map((phrase, index) => <div className={classnames('word-item', selectedPhrase.includes(phrase) && 'active')} key={phrase} onClick={() => selectPhrase(phrase)}>
      ...
      <div className="first-time-flow__account-password-btn">
        <Button type="primary" className="first-time-flow__button" disabled={disabled} onClick={handleNext}>
          {t('next')}
        </Button>
      </div>
    </div>;
