...
export default function ConfirmRecoveryPhrase({
  ...
  const [phraseElements, setPhraseElements] = useState(initializePhraseElements());
  const validate = useMemo(() => debounce(elements => {
    setMatching(Object.values(elements).join(' ') === seedPhrase);
  }, 500), [setMatching, seedPhrase]);
  ...
  return <div>
      ...
      <RecoveryPhraseChips seedPhrase={splitSeedPhrase} confirmPhase setInputValue={handleSetPhraseElements} inputValue={phraseElements} indicesToCheck={indicesToCheck} />
