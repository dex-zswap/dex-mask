...
import { FONT_WEIGHT, JUSTIFY_CONTENT, TEXT_ALIGN, TYPOGRAPHY } from '@view/helpers/constants/design-system';
...
export default function RecoveryPhrase({
  ...
  return <div>
      ...
      <Box justifyContent={JUSTIFY_CONTENT.CENTER} textAlign={TEXT_ALIGN.CENTER} marginBottom={4}>
      ...
      <Box justifyContent={JUSTIFY_CONTENT.SPACE_EVENLY} textAlign={TEXT_ALIGN.LEFT} marginBottom={4} className="recovery-phrase__tips">
      ...
      <RecoveryPhraseChips seedPhrase={seedPhrase.split(' ')} seedPhraseRevealed={seedPhraseRevealed} />
      <div className="recovery-phrase__footer">
        {seedPhraseRevealed ? <div className="recovery-phrase__footer--copy">
            <Button onClick={() => {
          handleCopy(seedPhrase);
        }} icon={copied ? null : <Copy size={20} color="#3098DC" />} className="recovery-phrase__footer--copy--button">
              {copied ? t('copiedExclamation') : t('copyToClipboard')}
            </Button>
            <Button rounded type="primary" className="recovery-phrase__footer--button" onClick={() => {
          history.push(INITIALIZE_CONFIRM_SEED_PHRASE_ROUTE);
        }}>
              {t('next')}
            </Button>
          </div> : <Button rounded type="primary" className="recovery-phrase__footer--button" onClick={() => {
