...
export default function CrossChainDestWrapper() {
  ...
  const targetIsNative = crossInfo.targetCoinAddress === ethers.constants.AddressZero;
  const {
    loading,
    error,
    res
    ...
    token_address: isNativeAsset ? ethers.constants.AddressZero : crossInfo.coinAddress
  }), [crossInfo.fromChain, crossInfo.coinAddress]);
  useDeepEffect(() => {
    if (!loading && !error && res?.c === 200) {
      const supportChains = res?.d.map(chain => ({ ...chain,
  ...
  return <div className="cross-chain-dest__wrapper">
      <div className="cross-chain-dest-type-selector">
        ...
        dispatch(updateCrossChainState({
          ...
          <img src={crossInfo.isInternalTrans ? 'images/dex/send/checked.png' : 'images/dex/send/checkbox.png'} alt="checkbox" />
          <span>{t('dex_trans')}</span>
        </div>
        {!crossInfo.isInternalTrans && <ChainSwitcher currentChainId={crossInfo.destChain} outSideChains={crossInfo.supportChains} onChange={(type, changedChainId) => {
        dispatch(updateCrossChainState({
      ...
      <div>
        {crossInfo.isInternalTrans ? <Dest /> : <EnsInput userInput={crossInfo.dest} chainId={crossInfo.destChain} className="send__to-row" onChange={userInputAddress => dispatch(updateCrossChainState({
        userInputAddress
      }))} onValidAddressTyped={dest => dispatch(updateCrossChainState({
        dest
      }))} selectedAddress={crossInfo.dest} onPaste={text => dispatch(updateCrossChainState({
        dest: text
      }))} onReset={() => dispatch(updateCrossChainState({
        dest: '',
        userInputAddress: ''
      }))} scanQrCode={() => {
