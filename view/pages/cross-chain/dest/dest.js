...
const CrossChainDest = () => {
  ...
  const chainChange = useCallback((chainType, chain) => {
    ...
    const targetInfo = supportChains.find(({
      meta_chain_id
    }) => toBnString(meta_chain_id) === toBnString(chain));
  ...
  return <div className="cross-chain-dest__component">
      <div className="top">
        <CurrentToken useOut coinAddress={crossInfo.targetCoinAddress} diameter={40} coinSymbol={crossInfo.targetCoinSymbol} currentChainId={crossInfo.destChain}>
          <AccountSwitcher address={crossInfo.dest} onChange={accountChange} />
        </CurrentToken>
        <ChainSwitcher currentChainId={crossInfo.destChain} outSideChains={crossInfo.supportChains} onChange={chainChange} />
      </div>
