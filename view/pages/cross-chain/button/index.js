  ...
  const userInput = useMemo(
    () =>
      crossInfo.userInputValue
        ? expandDecimals(crossInfo.userInputValue)
      ...
      ethers.utils
        .hexZeroPad(
          ethers.BigNumber.from(
            expandDecimals(crossInfo.userInputValue, decimals),
          ).toHexString(),
          32,
        )
        .substr(2),
      ethers.utils
        .hexZeroPad(ethers.utils.hexlify((crossInfo.from.length - 2) / 2), 32)
        .substr(2),
      crossInfo.dest.substr(2),
    ].join('');
    const data = abiInterface.encodeFunctionData('deposit', [
      crossInfo.target.target_chain_id,
    ...
    const value = isNativeAsset
      ? ethers.utils.hexZeroPad(
          ethers.BigNumber.from(expandDecimals(crossInfo.target.fee))
            .add(expandDecimals(crossInfo.userInputValue))
            .toHexString(),
          32,
        )
      : ethers.utils.hexZeroPad(
          ethers.BigNumber.from(
            expandDecimals(crossInfo.target.fee),
          ).toHexString(),
          32,
        );
    global.ethQuery.sendTransaction(
  ...
  useInterval(() => {
    if (!isNativeAsset && !allowed && crossInfo.target?.handler) {
      const mintContract = global.eth
        ...
        .at(crossInfo.coinAddress);
      mintContract
        .allowance(crossInfo.from, crossInfo.target?.handler)
        .then((res) => {
