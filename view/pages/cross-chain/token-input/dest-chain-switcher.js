...
const CrossDestChainSwitcher = () => {
  ...
  const destChainInfo = useMemo(() => allNetworks.find(({ networkId, chainId }) => (networkId === crossChainState.destChain || chainId === crossChainState.destChain)) ?? {}, [crossChainState.destChain, allNetworks])
      ...
      .then((res) => {
        ...
        if (res.c === 200) {
          res.d.forEach((bridge) => {
            find = allNetworks.find(({ networkId }) => networkId === bridge.target_meta_chain_id)
    ...
    <div className='cross-dest-chain-switcher base-width flex space-between items-center'>
      <div className='current-chain-info flex space-between items-center' onClick={toggleChainSwitcher}>
        <div className='chain-info'>
          <i className={classnames('network-icon', (destChainInfo?.provider ?? 'rpc').toLowerCase())}></i>
          {destChainInfo?.label}
        </div>
        {
          chains.length > 1 ?
              ...
              <div className='switcher-menu'>
                {
                  chains.map((chain) => {
                    return (
                      <div key={chain.chainId} className={classnames('switcher-menu-item chain-info flex items-center', chain.chainId === destChainInfo?.chainId ? 'selected' : '')} onClick={(e) => {
                        e.stopPropagation();
