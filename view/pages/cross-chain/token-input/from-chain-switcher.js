...
const CrossFromChainSwitcher = () => {
  ...
  const isNative = useMemo(() => ethers.constants.AddressZero === crossChainState.coinAddress, [crossChainState.coinAddress])

  const fromChainInfo = useMemo(() => allNetworks.find(({ chainId, networkId }) => (chainId === crossChainState.fromChain || networkId === crossChainState.fromChain)) ?? {}, [crossChainState.fromChain, allNetworks])

  const toggleChainSwitcher = useCallback(() => chains.length > 1 && setShowChainSwitcher(showChainSwitcher => !showChainSwitcher), [chains])
  ...
  const selectChain = useCallback((chain) => {
    ...
    .then((res) => {
      if (res.c === 200 && res.d.length) {
        dispatch(chain.isBulitIn ? setProviderType(chain.provider) : setRpcTarget(...chain.setPrcParams))
        .then(() => {
      ...
      .then((res) => {
        ...
        if (res.c === 200) {
          res.d.forEach((bridge) => {
            find = allNetworks.find(({ networkId }) => networkId === bridge.meta_chain_id)
  ...
  return (
    <div className='cross-from-chain-switcher base-width'>
      <div className='current-chain-info flex space-between items-center' onClick={toggleChainSwitcher}>
        <div className='chain-info'>
          <i className={classnames('network-icon', (fromChainInfo.provider ?? 'rpc').toLowerCase())}></i>
          {fromChainInfo.label}
        </div>
        {
          chains.length > 1 ?
          (
            <i className={classnames('drop-icon', showChainSwitcher ? 'open' : '')}></i>
          )
          :
          null
        }
        {
          ...
          (
            <>
              <div className='switcher-select-mask' onClick={(e) => {
                e.stopPropagation();
                toggleChainSwitcher();
              }}></div>
              <div className='switcher-menu'>
                {
                  chains.map((chain) => {
                    return (
                      <div key={chain.chainId} className={classnames('switcher-menu-item chain-info flex items-center', chain.chainId === fromChainInfo.chainId ? 'selected' : '')} onClick={(e) => {
                        e.stopPropagation();
                        selectChain(chain);
                        toggleChainSwitcher();
                      }}>
                        <i className={classnames('network-icon', (chain.provider ?? 'rpc').toLowerCase())}></i>
                        {chain.label}
                      </div>
                    )
                  })
