...
const CrossChainSplitor = () => {
  ...
  const reverseTokenAndChain = useCallback(() => {
    ...
    .then(async (res) => {
      if (res.c === 200) {
        const target = res.d.find(({ target_meta_chain_id, target_token_address }) => target_meta_chain_id === toBnString(crossInfo.fromChain) && target_token_address === crossInfo.coinAddress);
