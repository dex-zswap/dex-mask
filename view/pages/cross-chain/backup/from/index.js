...
import { setProviderType, showAccountDetail, updateCrossChainState } from '@view/store/actions';
...
const CrossChainFrom = () => {
  ...
  const groups = useMemo(() => {
    ...
    return res?.d.map(chain => ({ ...chain,
      chainId: chain.meta_chain_id
    }));
  }, [loading, error, res]);
  const chainChange = useCallback((chainType, chain) => {
    const targetChain = ethers.BigNumber.from(chain).toString();
    const targetInfo = groups.find(({
      chainId
    }) => chainId === targetChain);
  ...
  const outSideChains = useMemo(() => {
    return groups.filter(({
      chainId
    }) => toBnString(chainId) !== toBnString(crossInfo.destChain));
  }, [groups, crossInfo]);
  const tokenChange = useCallback(token => {
    checkTokenBridge({
      ...
      if (res.c === 200) {
        ...
        }) => toBnString(target_meta_chain_id) === toBnString(crossInfo.destChain));
  ...
  return <div className="cross-chain-from__wrapper">
      <div className="top">
        <CurrentToken selectable useOut coinAddress={crossInfo.coinAddress} diameter={40} coinSymbol={crossInfo.coinSymbol} currentChainId={crossInfo.fromChain} onChange={tokenChange}>
          <AccountSwitcher address={crossInfo.from} onChange={accountChange} />
        </CurrentToken>
        <ChainSwitcher currentChainId={crossInfo.fromChain} onChange={chainChange} outSideChains={outSideChains} />
      </div>
      <UserInputValue symbol={crossInfo.coinSymbol} coinAddress={crossInfo.coinAddress} tokenBalance={tokenBalance} />
