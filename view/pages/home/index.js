...
import { ALERT_TYPES, WEB3_SHIM_USAGE_ALERT_STATES } from '@shared/constants/alerts';
import { ENVIRONMENT_TYPE_NOTIFICATION, ENVIRONMENT_TYPE_POPUP } from '@shared/constants/app';
import { activeTabHasPermissions, getCurrentEthBalance, getFirstPermissionRequest, getInfuraBlocked, getIsMainnet, getAppState, getOriginOfCurrentTab, getTotalUnapprovedCount, getUnapprovedTemplatedConfirmations, getWeb3ShimUsageStateForOrigin, unconfirmedTransactionsCountSelector } from '@view/selectors';
import { getThreeBoxLastUpdated, restoreFromThreeBox, setAlertEnabledness, setConnectedStatusPopoverHasBeenShown, setDefaultHomeActiveTabName, setRecoveryPhraseReminderHasBeenShown, setRecoveryPhraseReminderLastShown, setShowRestorePromptToFalse, setWeb3ShimUsageAlertDismissed, turnThreeBoxSyncingOn } from '@view/store/actions';
...
export default function HomePage() {
  ...
  const shouldShowWeb3ShimUsageNotification = isPopup && useSelector(getWeb3ShimUsageAlertEnabledness) && useSelector(activeTabHasPermissions) && useSelector(state => getWeb3ShimUsageStateForOrigin(state, originOfCurrentTab)) === WEB3_SHIM_USAGE_ALERT_STATES.RECORDED;
  ...
  return <div className="main-container dex-page-container">
      <div className="home__container base-width">
        <div className="home__main-view">
          ...
          <Tabs defaultActiveTabName={t('assets')} tabsClassName="home__tabs">
            <Tab activeClassName="home__tab--active" className="home__tab" data-testid="home__asset-tab" name={t('assets')}>
              <AssetList onClickAsset={asset => history.push(`${ASSET_ROUTE}/${asset}`)} />
            </Tab>
            <Tab activeClassName="home__tab--active" className="home__tab" data-testid="home__activity-tab" name={t('activity')}>
              <TransactionList />
            </Tab>
          </Tabs>
        </div>
