...
import { getUnconnectedAccountAlertEnabledness, getUnconnectedAccountAlertShown } from './reducer/dexmask/dexmask';
...
async function startApp(dexmaskState, backgroundConnection, opts) {
  ...
  const currentLocaleMessages = dexmaskState.currentLocale ? await fetchLocale(dexmaskState.currentLocale) : {};
  ...
  const unapprovedTxsAll = txHelper(dexmaskState.unapprovedTxs, dexmaskState.unapprovedMsgs, dexmaskState.unapprovedPersonalMsgs, dexmaskState.unapprovedDecryptMsgs, dexmaskState.unapprovedEncryptionPublicKeyMsgs, dexmaskState.unapprovedTypedMessages, dexmaskState.network, dexmaskState.provider.chainId);
