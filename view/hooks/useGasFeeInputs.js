...
import { EDIT_GAS_MODES, GAS_ESTIMATE_TYPES, GAS_LIMITS } from '@shared/constants/gas';
import { conversionGreaterThan, conversionLessThan, multiplyCurrencies } from '@shared/modules/conversion.utils';
import { getMaximumGasTotalInHexWei, getMinimumGasTotalInHexWei } from '@shared/modules/gas.utils';
import { PRIMARY, SECONDARY } from '@view/helpers/constants/common';
import { GAS_FORM_ERRORS } from '@view/helpers/constants/gas';
import { bnGreaterThan, bnLessThan, bnLessThanEqualTo } from '@view/helpers/utils';
import { addHexes, decGWEIToHexWEI, decimalToHex, hexToDecimal, hexWEIToDecGWEI } from '@view/helpers/utils/conversions.util';
import { checkNetworkAndAccountSupports1559, getAdvancedInlineGasShown, getSelectedAccount, getShouldShowFiat } from '@view/selectors';
...
export function useGasFeeInputs(defaultEstimateToUse = 'medium', transaction, minimumGasLimit = '0x5208', editGasMode) {
  const {
    balance: ethBalance
  } = useSelector(getSelectedAccount);
  const networkAndAccountSupports1559 = useSelector(checkNetworkAndAccountSupports1559); // We need to know whether to show fiat conversions or not, so that we can
  // default our fiat values to empty strings if showing fiat is not wanted or
  // possible.

  const showFiat = useSelector(getShouldShowFiat); // We need to know the current network's currency and its decimal precision
  // to calculate the amount to display to the user.

  const {
    currency: primaryCurrency,
    numberOfDecimals: primaryNumberOfDecimals
  } = useUserPreferencedCurrency(PRIMARY); // For calculating the value of gas fees in the user's preferred currency we
  // first have to know what that currency is and its decimal precision

  const {
    currency: fiatCurrency,
    numberOfDecimals: fiatNumberOfDecimals
  } = useUserPreferencedCurrency(SECONDARY); // We need the gas estimates from the GasFeeController in the background.
  ...
  const [initialMaxFeePerGas] = useState(networkAndAccountSupports1559 && !transaction?.txParams?.maxFeePerGas ? Number(hexWEIToDecGWEI(transaction?.txParams?.gasPrice)) : Number(hexWEIToDecGWEI(transaction?.txParams?.maxFeePerGas)));
  const [initialMaxPriorityFeePerGas] = useState(networkAndAccountSupports1559 && !transaction?.txParams?.maxPriorityFeePerGas ? initialMaxFeePerGas : Number(hexWEIToDecGWEI(transaction?.txParams?.maxPriorityFeePerGas)));
  const [initialGasPrice] = useState(Number(hexWEIToDecGWEI(transaction?.txParams?.gasPrice)));
  const [initialMatchingEstimateLevel] = useState(transaction?.userFeeLevel || null);
  ...
  const [maxFeePerGas, setMaxFeePerGas] = useState(initialMaxFeePerGas && initialFeeParamsAreCustom ? initialMaxFeePerGas : null);
  const [maxPriorityFeePerGas, setMaxPriorityFeePerGas] = useState(initialMaxPriorityFeePerGas && initialFeeParamsAreCustom ? initialMaxPriorityFeePerGas : null);
  const [gasPriceHasBeenManuallySet, setGasPriceHasBeenManuallySet] = useState(initialMatchingEstimateLevel === 'custom');
  const [gasPrice, setGasPrice] = useState(initialGasPrice && initialFeeParamsAreCustom ? initialGasPrice : null);
  const [gasLimit, setGasLimit] = useState(Number(hexToDecimal(transaction?.txParams?.gas ?? '0x0')));
  const userPrefersAdvancedGas = useSelector(getAdvancedInlineGasShown);
  const dontDefaultToAnEstimateLevel = userPrefersAdvancedGas && transaction?.txParams?.maxPriorityFeePerGas && transaction?.txParams?.maxFeePerGas;
  const initialEstimateToUse = transaction ? initialMatchingEstimateLevel : defaultEstimateToUse;
  const [estimateToUse, setInternalEstimateToUse] = useState(dontDefaultToAnEstimateLevel ? null : initialEstimateToUse); // We specify whether to use the estimate value by checking if the state
  ...
  const maxFeePerGasToUse = maxFeePerGas ?? getGasFeeEstimate('suggestedMaxFeePerGas', gasFeeEstimates, gasEstimateType, estimateToUse, initialMaxFeePerGas);
  const maxPriorityFeePerGasToUse = maxPriorityFeePerGas ?? getGasFeeEstimate('suggestedMaxPriorityFeePerGas', gasFeeEstimates, gasEstimateType, estimateToUse, initialMaxPriorityFeePerGas);
  const [initialGasPriceEstimates] = useState(gasFeeEstimates);
  const gasPriceEstimatesHaveNotChanged = isEqual(initialGasPriceEstimates, gasFeeEstimates);
  ...
  if (networkAndAccountSupports1559) {
    gasSettings.maxFeePerGas = maxFeePerGasToUse ? decGWEIToHexWEI(maxFeePerGasToUse) : decGWEIToHexWEI(gasPriceToUse || '0');
    gasSettings.maxPriorityFeePerGas = maxPriorityFeePerGasToUse ? decGWEIToHexWEI(maxPriorityFeePerGasToUse) : gasSettings.maxFeePerGas;
    gasSettings.baseFeePerGas = decGWEIToHexWEI(gasFeeEstimates.estimatedBaseFee ?? '0');
  ...
  const maximumCostInHexWei = getMaximumGasTotalInHexWei(gasSettings); // If in swaps, we want to calculate the minimum gas fee differently than the max
  ...
  }] = useCurrencyDisplay(addHexPrefix(multiplyCurrencies(maxPriorityFeePerGasToUse, gasLimit, {
    toNumericBase: 'hex',
  ...
  }] = useCurrencyDisplay(maximumCostInHexWei, {
    numberOfDecimals: fiatNumberOfDecimals,
    currency: fiatCurrency,
    prefix: '$'
  ...
  }] = useCurrencyDisplay(minimumCostInHexWei, {
    numberOfDecimals: fiatNumberOfDecimals,
    currency: fiatCurrency,
    prefix: '$'
  });
  ...
  const gasLimitTooLow = conversionLessThan({
    ...
    value: minimumGasLimit || GAS_LIMITS.SIMPLE,
    fromNumericBase: 'hex'
  });
  ...
  switch (gasEstimateType) {
    case GAS_ESTIMATE_TYPES.FEE_MARKET:
      if (bnLessThanEqualTo(maxPriorityFeePerGasToUse, 0)) {
        gasErrors.maxPriorityFee = GAS_FORM_ERRORS.MAX_PRIORITY_FEE_BELOW_MINIMUM;
      } else if (!isGasEstimatesLoading && bnLessThan(maxPriorityFeePerGasToUse, gasFeeEstimates?.low?.suggestedMaxPriorityFeePerGas)) {
        gasWarnings.maxPriorityFee = GAS_FORM_ERRORS.MAX_PRIORITY_FEE_TOO_LOW;
      } else if (bnGreaterThan(maxPriorityFeePerGasToUse, maxFeePerGasToUse)) {
        gasErrors.maxFee = GAS_FORM_ERRORS.MAX_FEE_IMBALANCE;
      } else if (gasFeeEstimates?.high && bnGreaterThan(maxPriorityFeePerGasToUse, gasFeeEstimates.high.suggestedMaxPriorityFeePerGas * HIGH_FEE_WARNING_MULTIPLIER)) {
        gasWarnings.maxPriorityFee = GAS_FORM_ERRORS.MAX_PRIORITY_FEE_HIGH_WARNING;
      }

      if (!isGasEstimatesLoading && bnLessThan(maxFeePerGasToUse, gasFeeEstimates?.low?.suggestedMaxFeePerGas)) {
        gasWarnings.maxFee = GAS_FORM_ERRORS.MAX_FEE_TOO_LOW;
      } else if (gasFeeEstimates?.high && bnGreaterThan(maxFeePerGasToUse, gasFeeEstimates.high.suggestedMaxFeePerGas * HIGH_FEE_WARNING_MULTIPLIER)) {
  ...
  const errorsAndWarnings = { ...gasWarnings,
    ...gasErrors
  };
  const minimumTxCostInHexWei = addHexes(minimumCostInHexWei, transaction?.txParams?.value || '0x0');
