...
import { valuesFor } from '.';
export default function txHelper(unapprovedTxs, unapprovedMsgs, personalMsgs, decryptMsgs, encryptionPublicKeyMsgs, typedMessages, network, chainId) {
  ...
  const txValues = network ? valuesFor(unapprovedTxs).filter(txMeta => transactionMatchesNetwork(txMeta, chainId, network)) : valuesFor(unapprovedTxs);
